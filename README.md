...
TestGuard Agent 是一个面向 Python 项目的软件工程师 Agent 与权限隔离执行平台，目标是分析代码仓库、执行代码审查、生成自动修复计划、补齐单元测试、调用 LLM 生成 pytest，并在受限环境中安全执行和评估结果。
...
当前阶段已完成 SDD 规格初稿、可运行的自动测试执行闭环、Docker 权限隔离执行器、可离线演示的测试规划与生成 Agent、生成代码安全检查、结构化结果分析与 JSON 运行报告、失败诊断建议、LLM Prompt 导出、Benchmark 评估、代码审查 Agent、默认 dry-run 的自动修 Bug Agent、缺失覆盖单测生成 Agent、基于 LangGraph StateGraph 的软件工程师 Agent 编排入口，以及可接入 DashScope / DeepSeek 的 LLM 测试生成 Agent。
...
本项目第一阶段代码为课程项目自研实现。LLM 兼容调用参考 DashScope OpenAI 兼容接口与 DeepSeek API 官方文档。
