...
当前阶段先不依赖在线大模型，优先完成 SDD 规格初稿、可运行的自动测试执行闭环、Docker 权限隔离执行器、可离线演示的规则型测试生成 Agent，以及结构化结果分析与 JSON 运行报告。
...
当前测试生成器会分析 Python AST，识别公开函数，并生成 pytest 测试到临时工作区执行，不会修改原始项目源码。后续可将规则型生成器替换为 LLM Test Generator Agent。
