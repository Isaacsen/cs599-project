...
当前阶段先不依赖在线大模型，优先完成 SDD 规格初稿、可运行的自动测试执行闭环、Docker 权限隔离执行器、可离线演示的测试规划与生成 Agent、生成代码安全检查、结构化结果分析与 JSON 运行报告、失败诊断建议、LLM Prompt 导出、Benchmark 评估，以及可独立运行的代码审查 Agent。
