...
Write-Host "[TestGuard Final Verify] Run unit tests"
& $Python -m unittest discover -s tests
if ($LASTEXITCODE -ne 0) {
    throw "Unit tests failed."
}

Write-Host "`n[TestGuard Final Verify] Run compile check"
...
Write-Host "`n[TestGuard Final Verify] Export final report PDF"
& $Python scripts/export_report_pdf.py
if ($LASTEXITCODE -ne 0) {
    throw "Final report PDF export failed."
}

Write-Host "`n[TestGuard Final Verify] Inspect final report PDF"
...
Write-Host "`n[TestGuard Final Verify] Scan for possible plaintext API keys"
...
Write-Host "`n[TestGuard Final Verify] Done"
