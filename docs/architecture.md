...
- `llm_review` 后：固定进入 `llm_fix`，由 LLM Fix Agent 根据审查发现生成修复建议；显式传入 `--apply-fixes` 时才写回源码。
- `llm_tests` 后：启用 `--run-sandbox` 时进入 `sandbox_validate`，否则进入 `coverage_feedback`。
- `sandbox_validate` 后：进入 `repair_loop`。如果失败像代码缺陷，则把当前测试结果和失败诊断回送到 `llm_fix`；如果失败像生成测试自身的问题，则回到 `llm_tests`；如果测试通过、达到上限或需要人工判断，则进入 `coverage_feedback`。
...
- `repair_loop`：根据沙箱结果决定回跳 `llm_fix`、回跳 `llm_tests`，或结束循环进入覆盖反馈。
