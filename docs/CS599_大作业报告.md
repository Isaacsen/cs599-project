...
| 多 Agent 协作 | LLM Review、LLM Fix、LLM Test、Sandbox、Repair、Coverage |
...
- `repair_loop_node`: 根据沙箱结果决定回跳 `llm_fix`、回跳 `llm_tests`，或结束循环。
...
当沙箱测试通过时，repair loop 会输出无需继续迭代；当测试失败且未达到 `--repair-iterations` 上限时，repair loop 会先判断失败类型：像代码缺陷的问题返回给 `llm_fix`，像生成测试自身的问题返回给 `llm_tests`；达到上限后进入覆盖反馈并保留 blocked 状态。
