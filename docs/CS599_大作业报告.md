...
Software Engineer Agent 的目标是提供一个可运行、可观察、可审计的软件工程师 Agent 工作流。系统面向 Python 项目，自动完成仓库扫描、LLM 代码审查、修复目标规划、LLM 修复建议、LLM 单测生成、沙箱验证、失败回跳和覆盖反馈。

产品不追求替代开发者直接合并代码，而是提供“AI 建议 + 工程验证 + 权限隔离 + 报告审计”的闭环能力。默认模式为 dry-run，只有显式开启写回参数时才会修改目标项目。

#### 2.1.2 用户角色

| 角色 | 关注点 | 使用方式 |
| --- | --- | --- |
| 课程评审者 | 项目是否满足 Agentic AI、SDD、LangGraph、多 Agent、权限隔离和评估要求。 | 查看报告、运行 Demo、检查 GitHub 仓库。 |
| Python 开发者 | 快速获得代码审查、修复建议和测试补齐建议。 | 对目标项目运行 `src.engineer`。 |
| Agent 系统开发者 | 扩展新的 Agent、模型 provider、沙箱策略或 Benchmark。 | 修改 `src/agents/`、`src/workflow/`、`src/sandbox/`。 |

#### 2.1.3 使用场景

| 场景 | 输入 | 输出 |
| --- | --- | --- |
| 代码审查 | Python 项目路径、LLM API Key | LLM findings、严重级别、修复建议。 |
| 修复规划 | Review findings、沙箱失败上下文 | 本轮修复目标、修复顺序、剩余 findings 数量。 |
| 修复建议 | 源码上下文、选中 findings、失败诊断 | dry-run 修复建议或写回后的源码变更。 |
| 单测生成 | 扫描结果、函数列表、源码上下文 | pytest 测试文件内容。 |
| 沙箱验证 | 目标项目、生成测试、沙箱策略 | pytest 结果、失败诊断、安全检查结果。 |
| 报告审计 | LangGraph 状态对象 | JSON / Markdown 运行报告和 PDF 课程报告。 |
...
| 编号 | 需求 |
| --- | --- |
| NFR-1 | 默认 dry-run，不主动修改目标项目。 |
| NFR-2 | 生成测试和修复建议必须经过安全检查或 patch safety review。 |
| NFR-3 | 沙箱验证应使用临时工作区，降低对原项目的影响。 |
| NFR-4 | 报告不得包含 API Key 明文。 |
| NFR-5 | LLM、Docker 或扫描失败时，节点应返回结构化失败报告。 |
| NFR-6 | CLI 应提供实时 Agent 进度，避免长时间 LLM 调用被误判为卡死。 |
| NFR-7 | 工作流应支持无 API Key 降级运行，便于课程评审复现基础流程。 |

#### 2.1.6 约束条件

- 当前主要支持 Python 项目。
- LLM 调用依赖用户配置环境变量，例如 `DASHSCOPE_API_KEY` 或 `DEEPSEEK_API_KEY`。
- Docker sandbox 需要本机 Docker 可用；否则可切换 local executor 或跳过 sandbox。
- `--apply-fixes` 和 `--apply-tests` 会写回目标项目，默认不启用。
- PDF 课程报告由 Markdown 导出，不属于 Agent 主流程的直接输出。

#### 2.1.7 验收标准

| 验收项 | 标准 |
| --- | --- |
| 主流程可运行 | `python -m src.engineer examples/review_target` 能完成并输出报告。 |
| 多 Agent 可观察 | Agent Timeline 包含 scan、llm_review、llm_fix_plan、llm_fix、llm_tests、coverage_feedback 等节点。 |
| 权限隔离可验证 | 启用 `--run-sandbox` 后能进入 sandbox validation。 |
| 结构化失败报告 | 缺少 API Key 或沙箱异常时不会直接崩溃，而是记录状态。 |
| 测试通过 | `python -m pytest tests -q` 通过。 |
| PDF 合规 | PDF 包含单独封面页、可点击跳转目录、书签和七个章节。 |
...
#### 2.2.2 LangGraph 状态对象

`SoftwareEngineerGraphState` 是工作流中的共享状态，核心字段如下：

| 字段 | 类型/内容 | 说明 |
| --- | --- | --- |
| `project_path` | string | 目标 Python 项目路径。 |
| `scan` | `RepositoryScanResult` | Repo Scan Agent 输出。 |
| `llm_review` | `LLMCodeReviewReport` | LLM 审查结果。 |
| `llm_fix_plan` | `LLMFixPlan` | 本轮修复目标选择。 |
| `llm_fix` | `LLMCodeFixReport` | LLM 修复建议和 patch review。 |
| `llm_tests` | `LLMTestGenerationReport` | LLM 生成测试。 |
| `sandbox_validation` | `SandboxValidationReport` | 沙箱执行和失败诊断。 |
| `repair_loop` | `RepairLoopReport` | 下一步回跳或结束决策。 |
| `coverage_feedback` | `CoverageFeedbackReport` | 覆盖反馈。 |
| `node_trace` | list | 已执行节点序列。 |
| `attempted_finding_indexes` | list | 已尝试处理的 findings。 |
| `resolved_finding_indexes` | list | 已验证解决的 findings。 |
| `status` | string | 最终状态。 |
...
| Agent | 输入 | 输出 |
| --- | --- | --- |
| Repo Scan Agent | `project_path` | `RepositoryScanResult`，包含源码、测试、配置、依赖、入口点和 issues。 |
| LLM Review Agent | `project_path`、`scan` | `LLMCodeReviewReport`，包含 findings。 |
| Fix Planner Agent | `llm_review`、`sandbox_validation`、已尝试 findings | `LLMFixPlan`，包含目标 indexes 和 rationale。 |
| Code Fix Agent | `scan`、`llm_review`、`llm_fix_plan`、沙箱反馈 | `LLMCodeFixReport`，包含修复建议、写回状态和 patch review。 |
| Test Writer Agent | `scan`、源码上下文、函数列表 | `LLMTestGenerationReport`，包含 pytest 内容和安全检查。 |
| Sandbox Agent | 项目、生成测试、executor 策略 | `SandboxValidationReport`，包含 pytest 结果和诊断。 |
| Repair Loop Agent | `sandbox_validation`、当前迭代次数 | `RepairLoopReport`，包含下一步节点。 |
| Coverage Agent | `scan`、`llm_tests` | `CoverageFeedbackReport`，包含覆盖率和缺失函数。 |

#### 2.2.4 条件路由规则

| 起点 | 条件 | 下一步 |
| --- | --- | --- |
| `llm_tests` | `--run-sandbox` 开启 | `sandbox_validate` |
| `llm_tests` | 未开启 sandbox | `coverage_feedback` |
| `sandbox_validate` | 总是 | `repair_loop` |
| `repair_loop` | 失败像代码缺陷 | `llm_fix_plan` |
| `repair_loop` | 失败像生成测试问题 | `llm_tests` |
| `repair_loop` | 测试通过但仍有未尝试 findings | `llm_fix_plan` |
| `repair_loop` | 测试通过、达到上限或需人工处理 | `coverage_feedback` |

#### 2.2.5 失败处理策略

| 失败类型 | 处理策略 |
| --- | --- |
| 扫描路径不存在 | Repo Scan Agent 返回 `status=failed` 和 `error_summary`。 |
| 缺少 API Key | LLM 节点返回 `skipped_missing_api_key`。 |
| LLM 请求异常 | LLM 节点返回 `status=failed` 和错误摘要。 |
| LLM 输出格式错误 | 解析失败时返回空结果或 fallback 计划。 |
| 生成测试安全检查失败 | 阻止写回，报告 `security_failed`。 |
| patch safety review 失败 | 阻止源码写回，报告违规原因。 |
| sandbox executor 异常 | Sandbox Agent 返回结构化失败报告。 |
| repair 次数达到上限 | Repair Loop 返回 `blocked` 或进入人工处理建议。 |

#### 2.2.6 权限边界

- LLM API Key 只从环境变量读取。
- 默认 dry-run，不写回源码或测试。
- 写回源码前执行 patch safety review。
- 生成测试先经过 Security Checker。
- 沙箱验证使用临时工作区。
- Docker executor 限制网络、资源、capabilities 和容器写权限。
...
| `--events-output` | 否 | Agent 事件 JSONL 输出路径，供 Web Viewer 回放 Timeline。 |
...
| `--docker-image` | 否 | Docker sandbox 镜像名称。 |
| `--timeout` | 否 | sandbox 执行超时时间，默认 30 秒。 |
| `--llm-timeout` | 否 | 单次 LLM 请求超时时间。 |
| `--llm-retries` | 否 | LLM 请求重试次数。 |
| `--repair-iterations` | 否 | repair loop 最大重试次数，默认 3。 |
| `--llm-test-file` | 否 | LLM 生成测试写回路径。 |
| `--max-functions` | 否 | 生成测试时最多考虑的公开函数数量。 |
| `--no-stream` | 否 | 关闭 Agent 级别流式进度。 |
| `--stream-llm-tokens` | 否 | 兼容旧参数；当前 token 级输出默认开启。 |
...
| 环境变量 | 说明 |
| --- | --- |
| `LLM_PROVIDER` | LLM provider，例如 `dashscope` 或 `deepseek`。 |
| `DASHSCOPE_API_KEY` | DashScope API Key。 |
| `DEEPSEEK_API_KEY` | DeepSeek API Key。 |
| `LLM_API_KEY` | OpenAI-compatible fallback API Key。 |
| `LLM_MODEL` | 模型名称，例如 `glm-5.2`。 |
| `LLM_BASE_URL` | 自定义 OpenAI-compatible endpoint。 |
| `LLM_TIMEOUT_SECONDS` | LLM 请求超时时间。 |
| `LLM_MAX_RETRIES` | LLM 请求最大重试次数。 |

#### 2.3.5 JSON 报告结构

`docs/runs/software_engineer.json` 的顶层字段包括：

| 字段 | 说明 |
| --- | --- |
| `project_path` | 目标项目路径。 |
| `status` | 工作流最终状态。 |
| `graph_runtime` | `langgraph` 或 fallback runtime。 |
| `node_trace` | Agent 执行轨迹。 |
| `summary` | 汇总指标。 |
| `scan` | Repo Scan Agent 结构化结果。 |
| `llm_review` | LLM 审查结果。 |
| `llm_fix_plan` | 修复规划结果。 |
| `llm_fix` | 修复建议结果。 |
| `llm_tests` | 测试生成结果。 |
| `sandbox_validation` | 沙箱验证结果。 |
| `repair_loop` | repair loop 决策。 |
...
#### 2.3.6 Markdown / PDF 输出

- `docs/runs/software_engineer.md` 是 Agent 运行报告，记录每个 Agent 的执行结果。
- `docs/CS599_大作业报告.md` 是课程最终报告源文件，包含封面、目录和七个章节。
- `docs/CS599_大作业报告.pdf` 由 `scripts/export_report_pdf.py` 导出，封面单独成页，目录可点击跳转，并包含 PDF 书签导航。
...
项目开发过程中使用 Codex 辅助完成需求拆分、代码修改、测试执行、文档检查和 Git 提交同步。AI IDE 的作用不是直接替代工程判断，而是帮助快速定位跨文件问题、持续运行验证命令，并在多轮迭代中保持项目架构一致。

![AI IDE 使用过程](images/ai_ide.png)
...
Demo 展示包括命令行运行和 Web 可视化两部分。命令行用于验证 Agent 主流程、`[agent-stream]` 与 `[llm-stream]` 输出；Web 页面用于展示 Agent 状态图、Timeline 和每轮中间输出。

Web 页面 Agent 状态图：

![Web 页面 Agent 状态图](images/web_agent_graph.png)

Timeline 事件流：

![Timeline 事件流](images/timeline.png)

LLM token 级实时输出：

![LLM token stream](images/token_stream.png)

静态流程图位于 `docs/runs/software_engineer_agent_flow.png`，运行报告位于 `docs/runs/software_engineer.md` 和 `docs/runs/software_engineer.json`。
...
| Report | 在现有 Web UI 基础上增加历史 run 对比、Trace Viewer 和 LLMOps 面板。 |
...
5. 增强前端可视化界面，增加 run 历史对比、失败详情过滤和报告导出入口。
...
建议后续课程进一步增加企业真实场景案例和 Agent 评估指标讲解，例如如何评估 Agent 的任务完成率、修复正确率、回归测试质量和失败恢复能力。同时也希望增加 LLMOps 与安全沙箱实践内容，让学生不仅关注模型能否生成结果，也关注生成结果是否可验证、可审计、可隔离。课堂 Demo 互评环节如果能够保留，会更有助于比较不同 Agent 架构在工程实现上的差异。
