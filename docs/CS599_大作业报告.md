...
TestGuard Agent 的目标是构建一个面向 Python 项目的自动测试智能体：系统能够扫描代码仓库，规划测试场景，生成 pytest 测试，在权限隔离沙箱中执行测试，执行代码审查，生成自动修 Bug 计划，生成缺失覆盖单测，并输出结构化报告、Benchmark 指标、失败诊断和后续 LLM Prompt 工件。
