...
当沙箱测试通过时，repair loop 会先检查是否还有未处理的 LLM findings；如果还有剩余问题，则回到 `llm_fix_plan` 选择下一批修复目标，继续执行 `llm_fix -> llm_tests -> sandbox_validate`。当测试失败且未达到 `--repair-iterations` 上限时，repair loop 会判断失败类型：像代码缺陷的问题返回给 `llm_fix_plan` 重新选择修复目标和顺序，再进入 `llm_fix`，像生成测试自身的问题返回给 `llm_tests`；达到上限后进入覆盖反馈并保留 blocked 状态。CLI 默认输出 `[agent-stream]` 节点开始与完成事件，用于观察长时间 LLM 调用进度。
