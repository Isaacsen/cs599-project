...
TestGuard Agent 的目标是构建一个面向 Python 项目的自动测试智能体：系统能够扫描代码仓库，规划测试场景，生成 pytest 测试，在权限隔离沙箱中执行测试，并输出结构化报告、Benchmark 指标、失败诊断和后续 LLM Prompt 工件。
...
本项目选择先实现可离线运行的规则型 Agent 闭环，保证 Demo 稳定；再提供 LLM Prompt 导出能力，为后续接入 DeepSeek、OpenAI 或 Ollama 留出扩展点。
...
当前系统已经支持导出 LLM Prompt。下一步可以接入 DeepSeek、OpenAI 或 Ollama，将规则型 Test Generator 替换为 LLM 生成器。
