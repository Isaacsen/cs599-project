...
flowchart TD
    ...
    B --> C["Rule Code Reviewer Agent"]
    C --> D["LLM Code Reviewer Agent"]
    D --> E["Bug Fixer Agent"]
    E --> F["Patch Reviewer Agent"]
    F --> G["Unit Test Writer Agent"]
    G --> H["LLM Test Generator Agent"]
    H --> I["Security Checker Agent"]
    I --> J["Sandbox Validator Agent"]
    J --> K["Docker Sandbox Executor"]
    K --> L["权限隔离策略"]
    L --> M["网络禁用 / 只读挂载 / 资源限制"]
    M --> N["Repair Loop Agent"]
    N --> O["Coverage Feedback Agent"]
    O --> P["Software Engineer JSON / Markdown Report"]
    H --> Q["OpenAI-compatible LLM Client"]
    Q --> R["DashScope / DeepSeek / OpenAI-compatible"]
```

### 3.2 Agent 交互流程

1. Repo Scanner 识别源码文件和测试文件。
2. Code Reviewer Agent 执行 AST 规则审查。
3. LLM Code Reviewer Agent 调用真实模型执行语义审查。
4. Bug Fixer Agent 默认生成 dry-run 修复计划。
5. Patch Reviewer Agent 对修复计划进行二次门禁。
6. Unit Test Writer Agent 生成缺失覆盖单测。
7. LLM Test Generator Agent 通过 DashScope / DeepSeek 兼容接口生成 pytest。
8. Security Checker Agent 对生成测试执行 AST 安全检查。
9. Sandbox Validator Agent 将生成测试放入临时工作区。
10. Docker Sandbox Executor 在受限容器中执行 pytest。
11. Result Analyzer 和 Failure Diagnoser 解析沙箱验证结果。
12. Repair Loop Agent 根据 Patch 审查和沙箱结果规划下一步修复。
13. Coverage Feedback Agent 汇总函数覆盖比例。
14. Report Writer 输出统一 JSON 和可观看 Markdown 报告。
