...
- `llm_fix_plan_node`: 优先调用真实 LLM 从 findings 和沙箱反馈中选择本轮一个或多个修复目标，并给出修复顺序；LLM 不可用时降级到规则排序。
- `llm_fix_node`: 调用真实 LLM 生成源码修复建议，必要时接收沙箱失败结果；写回前执行本地 patch safety review，拦截危险调用、语法错误和公共函数删除。
...
当沙箱测试通过时，repair loop 会先检查是否还有未尝试的 LLM findings；如果还有剩余问题，则回到 `llm_fix_plan` 选择下一批修复目标，继续执行 `llm_fix -> llm_tests -> sandbox_validate`。系统区分 attempted 与 resolved：dry-run 或仅生成修复建议只算 attempted，只有 `--apply-fixes` 写回源码、通过 patch safety review 且通过沙箱验证后才算 resolved。当测试失败且未达到 `--repair-iterations` 上限时，repair loop 会判断失败类型：像代码缺陷的问题返回给 `llm_fix_plan` 重新选择修复目标和顺序，再进入 `llm_fix`，像生成测试自身的问题返回给 `llm_tests`；达到上限后进入覆盖反馈并保留 blocked 状态。CLI 默认输出 `[agent-stream]` 节点开始与完成事件，并默认开启 `[llm-stream]` token 级模型输出；可通过 `--no-stream` 和 `--no-llm-token-stream` 关闭对应日志，并支持 `--llm-timeout` 与 `--llm-retries` 控制 LLM 请求。JSON 报告默认记录 replacement 摘要、长度和 SHA-256，不保存完整替换源码。
