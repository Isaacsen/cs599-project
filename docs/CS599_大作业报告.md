...
12. Software Engineer Agent 串联审查、修复计划和单测生成，输出统一 JSON 报告。
...
该模块统一编排 Code Reviewer、Bug Fixer 和 Unit Test Writer，输出包含审查发现、修复计划和生成测试内容的 JSON 报告。默认 dry-run；如需写回，可分别通过 `--apply-fixes` 和 `--apply-tests` 控制。
